...
Tenant behavior defaults are `enabled_agents=[SALE,SUPPORT]`, `campaign_routes=[]`, `source_routes=[]`, `intent_confidence_threshold=0.75`, and `route_stickiness_hours=24`. The former `default_agent` key is removed by migration 029. `client.reception_question` is a tenant-overridable multilingual template.
