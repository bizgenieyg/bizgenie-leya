...
import { enqueueMessage, settleAllOutboundQueues } from '../workers/outbound-queue.js';
...
test('owner reply: short GOWS ID, delivery before closure, quoted confirmation only and tenant isolation', async () => {
  ...
  await createEscalation(h.db, h.provider, input, h.settings); await settleAllOutboundQueues();
  ...
  await handleOwnerMessage(h.db, h.provider, randomUUID(), 'session', owner, 'Ответ', replyId(h.sent[1]!.id), h.settings); await settleAllOutboundQueues();
  e = await h.escalation(e.id); assert.equal(e.status, 'pending'); assert.equal(h.sent.length, 2);

  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, 'Завтра отвечу', e.owner_message_ids[0]!, h.settings); await settleAllOutboundQueues();
...
test('failed client delivery leaves escalation open and never offers learning', async () => {
  const h = await pgHarness();
  const input = { tenant_id: h.tenant, conversation_id: h.conversationId, client_chat_id: customer, client_name: 'Тестовый клиент', question: 'Можно завтра?', session: 'session', inbound_id: 'incoming' };
  await createEscalation(h.db, h.provider, input, h.settings); h.fail(); await settleAllOutboundQueues();
  ...
  let e = await h.escalation((await h.pg.query<{ id: string }>('select id from escalations limit 1')).rows[0]!.id);
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, 'Ответ', e.owner_message_ids[0]!, h.settings); await settleAllOutboundQueues();
...
test('createEscalation with an all-day-off schedule promises a callback and escalates now', async () => {
  ...
  await createEscalation(h.db, h.provider, input, closed); await settleAllOutboundQueues();
...
test('takeover, tenant pause and explicit resume remain separate', async () => {
  ...
  await createEscalation(h.db, h.provider, input, h.settings); await settleAllOutboundQueues();
  let e = await h.escalation((await h.pg.query<{ id: string }>('select id from escalations limit 1')).rows[0]!.id);
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, 'Беру на себя', e.owner_message_ids[0]!, h.settings); await settleAllOutboundQueues();
  assert.equal((await h.conversation()).bot_paused, true);
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, 'Ответ', e.owner_message_ids[0]!, h.settings); await settleAllOutboundQueues();
  e = await h.escalation(e.id); assert.equal(e.status, 'pending');
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, 'Пауза всё', null, h.settings); await settleAllOutboundQueues();
  assert.equal((await h.notificationSettings()).auto_replies_paused, true);
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, 'Продолжить всё', null, h.settings); await settleAllOutboundQueues();
  assert.equal((await h.conversation()).bot_paused, true);
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, 'Продолжить', e.owner_message_ids[0]!, h.settings); await settleAllOutboundQueues();
...
test('manual fromMe send pauses dialogue and closes pending escalation; API send is ignored', async () => {
  ...
  assert.equal(await observeOwnerOutgoing(h.db, h.tenant, body, new Date('2026-09-08T20:00:00Z')), true); await settleAllOutboundQueues();
  assert.equal((await h.conversation()).bot_paused, true);
...
test('quiet queue uses Jerusalem time, delivers each once across concurrent schedulers', async () => {
  ...
  await runDueScheduledEscalations(h.db, () => h.provider, new Date('2026-09-08T18:00:00Z')); await settleAllOutboundQueues();
  assert.equal(h.sent.length, 0);
  await Promise.all([runDueScheduledEscalations(h.db, () => h.provider, new Date('2026-09-09T06:00:00Z')), runDueScheduledEscalations(h.db, () => h.provider, new Date('2026-09-09T06:00:00Z'))]); await settleAllOutboundQueues();
...
test('due escalation is cancelled after owner activity and expires after maximum age', async () => {
  for (const kind of ['owner', 'expired']) {
    ...
    await runDueScheduledEscalations(h.db, () => h.provider, new Date('2026-09-08T13:00:00Z')); await settleAllOutboundQueues();
    assert.equal(h.sent.length, 0);
...
test('owner phone must differ from session; one-time numeric code binds LID and is single-use', async () => {
  ...
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', '88888888@lid', ` ${result.code}. `, null, settings); await settleAllOutboundQueues();
  let row = await h.notificationSettings();
  assert.equal(row.owner_chat_id, '88888888@lid'); assert.equal(row.owner_pairing_hash, null);
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', '77777777@lid', result.code, null, settings); await settleAllOutboundQueues();
...
test('a customer\'s six-digit message is not swallowed as a pairing attempt when no pairing is pending', async () => {
  ...
  const handled = await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', customer, '123456', null, settings); await settleAllOutboundQueues();
...
test('a wrong code or the right code from an unrelated sender never binds pairing', async () => {
  ...
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', '55555555@c.us', wrongCode, null, settings); await settleAllOutboundQueues();
  assert.equal((await h.notificationSettings()).owner_chat_id, null);
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', '55555555@c.us', result.code, null, settings); await settleAllOutboundQueues();
...
test('owner phone accepts explicit international country codes from Israel and other countries', async () => {
  ...
  const settings = { ...h.settings, ...(await h.notificationSettings()) } as unknown as OwnerSettings;
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', '972523695741@c.us', result.code, null, settings); await settleAllOutboundQueues();
...
test('complete real GOWS client and owner reply payloads traverse worker filters through delivery', async () => {
  ...
  await handleWebhookEvent(h.tenant, body, h.db, h.provider, null); await settleAllOutboundQueues();
  await handleWebhookEvent(h.tenant, body, h.db, h.provider, null); await settleAllOutboundQueues();
  ...
  await handleWebhookEvent(h.tenant, reply, h.db, h.provider, null); await settleAllOutboundQueues();
...
test('observeOwnerOutgoing guard #1 is idempotent on a re-delivered waha_msg_id', async () => {
  ...
  await enqueueMessage(h.db,h.tenant,h.provider,{session:'session',chatId:customer,text:'Pending bot reply'},
    {kind:'reply',notBefore:new Date(Date.now()+60_000)});
  ...
  assert.equal(await observeOwnerOutgoing(h.db, h.tenant, body, new Date('2026-09-08T20:05:00Z')), false); await settleAllOutboundQueues();
...
test('returning contact with assistant_introduced_at gets no repeated greeting', async () => {
  ...
  const run = async (introduced: boolean) => {
    ...
    await handleWebhookEvent(h.tenant, body, h.db, h.provider, ai as never); await settleAllOutboundQueues();
...
test('owner can pause any dialogue without an escalation; foreign dialogue cannot be changed', async () => {
  ...
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, `Беру на себя ${h.conversationId}`, null, h.settings); await settleAllOutboundQueues();
  assert.equal((await h.conversation()).bot_paused, true);
  await handleOwnerMessage(h.db, h.provider, otherTenant.id, 'session', owner, `Продолжить диалог ${h.conversationId}`, null, h.settings); await settleAllOutboundQueues();
  assert.equal((await h.conversation()).bot_paused, true, 'a different tenant id cannot touch this conversation');
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, `Продолжить диалог ${h.conversationId}`, null, h.settings); await settleAllOutboundQueues();
  assert.equal((await h.conversation()).bot_paused, false);
});
...
test('owner translation is opt-in: disabled makes zero model calls', async () => {
  for (const enabled of [false, true]) {
    ...
    const input = { tenant_id: h.tenant, conversation_id: h.conversationId, client_chat_id: customer, client_name: 'Тестовый клиент', question: 'Можно завтра?', session: 'session', inbound_id: 'incoming' };
    await createEscalation(h.db, h.provider, input, settings); await settleAllOutboundQueues();
...
test('timeout reminder is once, quoted reply matches it, quiet hours do not count, closure follows delivery', async () => {
  ...
  await runEscalationTimeouts(h.db, () => h.provider, new Date('2026-09-09T06:29:00Z')); await settleAllOutboundQueues(); assert.equal(h.sent.length, 0);
  await runEscalationTimeouts(h.db, () => h.provider, new Date('2026-09-09T06:30:00Z')); await settleAllOutboundQueues(); assert.equal(h.sent.length, 1);
  assert.equal((await h.escalation(escalationId)).owner_message_ids.length, 2);
  await runEscalationTimeouts(h.db, () => h.provider, new Date('2026-09-09T06:40:00Z')); await settleAllOutboundQueues(); assert.equal(h.sent.length, 1);
  await runEscalationTimeouts(h.db, () => h.provider, new Date('2026-09-09T07:30:00Z')); await settleAllOutboundQueues(); assert.equal(h.sent.length, 2);
...
test('voice runs full GOWS identity, quota, transcription, agent, FAQ pipeline and wipes buffer', async () => {
  ...
  await handleVoiceUsage(h.db, voiceRouting(h), body, h.provider, { async transcribe() { calls++; return { text: 'Какая цена?', confidence: 0.99, ambiguous: false, language: 'ru' }; } }, { async download() { return bytes; } }); await settleAllOutboundQueues();
...
test('voice pause and quota stop STT; uncertainty asks for clarification without FAQ', async () => {
  ...
  for (const mode of ['paused', 'denied', 'uncertain', 'disabled']) {
    ...
    const body = JSON.parse(readFileSync('src/services/fixtures/gows-incoming-lid.json', 'utf8'));
    body.payload.from = customer; body.payload._data.Info.Chat = customer; body.payload.body = null; body.payload.hasMedia = true; body.payload.media = { mimetype: 'audio/wav', url: 'http://internal/api/files/session/id.wav' };
    await handleVoiceUsage(h.db, voiceRouting(h), body, h.provider, mode === 'disabled' ? null : { async transcribe() { calls++; return { text: 'Неясно завтра', confidence: 0.4, ambiguous: true, language: 'ru' }; } }, { async download() { downloads++; return wav(); } }); await settleAllOutboundQueues();
    ...
    if (mode === 'paused') {
      ...
      assert.match(h.sent.filter(m => m.chatId === customer).at(-1)!.text, mode === 'uncertain' ? /уточните/ : mode === 'denied' ? /недоступны/ : /не удалось распознать/);
    }
  }
});
...
test('voice opt-out exits before download, STT, admission or client response', async () => {
  ...
  await handleVoiceUsage(h.db, voiceRouting(h), body, h.provider, { async transcribe() { transcriptions++; return { text: 'Текст', confidence: 1, ambiguous: false, language: 'ru' }; } }, { async download() { downloads++; return wav(); } }); await settleAllOutboundQueues();
...
async function ownerAnswerRun(settings: Partial<OwnerSettings>, question: string, answer: string, ai: Parameters<typeof handleOwnerMessage>[8]) {
  ...
  await createEscalation(h.db, h.provider, input, { ...h.settings, ...settings }); await settleAllOutboundQueues();
  const e = await h.escalation((await h.pg.query<{ id: string }>('select id from escalations limit 1')).rows[0]!.id);
  await handleOwnerMessage(h.db, h.provider, h.tenant, 'session', owner, answer, e.owner_message_ids[0]!, { ...h.settings, ...settings }, ai); await settleAllOutboundQueues();
  const delivered = await h.escalation(e.id);
  return { h, delivered, text: h.sent.filter(m => m.chatId === customer).at(-1)!.text };
}

const approving = (text: string) => ({ async generateReply(input: { systemPrompt: string }) {
  return { text: input.systemPrompt.startsWith('Ты проверяющий') ? '{"adds_facts":false,"changes_meaning":false}' : text }; } });
...
test('polish: invented price or flipped meaning falls back to the owner text', async () => {
  const price = await ownerAnswerRun({}, 'Можно в субботу?', 'да', approving('Да, в субботу можно, стоимость 200 шекелей.'));
  assert.match(price.text, /Передаю ответ владельца/); assert.match(price.text, /«да»/);
  const known = await ownerAnswerRun({}, 'Сколько стоит маникюр?', 'как в прайсе', approving('Маникюр стоит 120 шекелей.'));
  assert.equal(known.text, 'Маникюр стоит 120 шекелей.');
  const flipped = await ownerAnswerRun({}, 'Можно в субботу?', 'нет', approving('Да, приходите в субботу.'));
...
test('polish: simulator returns exactly the text real delivery sends for the same input', async () => {
  ...
  const ai = { async generateReply(input: { systemPrompt: string; userMessage: string }) {
    if (input.systemPrompt.startsWith('Ты проверяющий')) return { text: '{"adds_facts":false,"changes_meaning":false}' };
    return { text: JSON.parse(input.userMessage).ownerAnswer === 'да' ? 'Да, в субботу можно.' : 'Уточню.' }; } };
...
test('polish: reviewer example with an invented visit falls back to the original owner text in real delivery', async () => {
  const ai = { async generateReply(input: { systemPrompt: string }) {
    return { text: input.systemPrompt.startsWith('Ты проверяющий') ? '{"adds_facts":true,"changes_meaning":false}' : 'Да, можно, мастер приедет к вам домой.' }; } };
  const run = await ownerAnswerRun({}, 'Можно записаться?', 'да', ai);
  assert.match(run.text, /Передаю ответ владельца:\s+«да»/);
  const garbage = await ownerAnswerRun({}, 'Можно записаться?', 'да', { async generateReply(input: { systemPrompt: string }) {
    return { text: input.systemPrompt.startsWith('Ты проверяющий') ? 'ok' : 'Да, можно.' }; } });
  assert.match(garbage.text, /«да»/);
});

test('echo of our own send before messages.waha_msg_id is stamped never pauses the dialogue', async () => {
  const h = await pgHarness();
  const echo = (id: string, text: string) => ({ event: 'message', payload: { id, from: '972500000009@c.us', to: customer, fromMe: true, source: 'app', body: text, _data: { Info: { IsFromMe: true, Chat: customer } } } });
  // Stage 1: the sender stamped outbound_messages.waha_msg_id, the linked messages row is not stamped yet.
  await h.pg.query("insert into outbound_messages(tenant_id,session,chat_id,text,kind,priority,status,dedupe_key,waha_msg_id,started_at) values($1,'session',$2,'Ответ бота','reply',0,'sent','echo-1',$3,now())", [h.tenant, customer, `true_${customer}_ECHO1`]);
  assert.equal(await observeOwnerOutgoing(h.db, h.tenant, echo(`true_${customer}_ECHO1`, 'Ответ бота')), false);
  // Stage 2: the echo overtakes even sendText's return — row still `sending`, matched by contact and text.
  await h.pg.query("insert into outbound_messages(tenant_id,session,chat_id,text,kind,priority,status,dedupe_key,started_at) values($1,'session',$2,'Второй ответ','reply',0,'sending','echo-2',now())", [h.tenant, customer]);
  assert.equal(await observeOwnerOutgoing(h.db, h.tenant, echo('UNKNOWN2', 'Второй ответ')), false);
  assert.equal((await h.conversation()).bot_paused, false);
