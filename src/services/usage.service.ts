...
export async function recordUsageEvent(db:DatabaseClient,input:{tenantId:string;eventType:string;quantity?:number;eventKey?:string;metadata?:Record<string,unknown>}):Promise<void> {
  try {
    const {error}=await db.from('usage_events').insert({tenant_id:input.tenantId,event_type:input.eventType,quantity:input.quantity??1,
      ...(input.eventKey?{event_key:input.eventKey}:{}),metadata:input.metadata??{}});
...
export async function admitUsage(db:DatabaseClient,tenantId:string,eventKey:string,messages=1,voiceSeconds=0):Promise<UsageAdmission> {
  try {
    const {data,error}=await db.rpc('admit_tenant_usage',{p_tenant_id:tenantId,p_event_key:eventKey,p_messages:messages,p_voice_seconds:voiceSeconds,p_default_messages:BASIC_USAGE_LIMITS.messagesPerMonth,p_default_voice_seconds:BASIC_USAGE_LIMITS.voiceSecondsPerMonth});
...
export async function usageSummary(db:DatabaseClient,tenantId:string){
  const {data,error}=await db.rpc('tenant_usage_summary',{p_tenant_id:tenantId,p_default_messages:BASIC_USAGE_LIMITS.messagesPerMonth,p_default_voice_seconds:BASIC_USAGE_LIMITS.voiceSecondsPerMonth});
