...
export async function admitUsage(db:DatabaseClient,tenantId:string,eventKey:string,messages=1,voiceSeconds=0):Promise<UsageAdmission> {
  try {
    const {data,error}=await db.rpc('admit_tenant_usage',{p_tenant_id:tenantId,p_event_key:eventKey,p_messages:messages,p_voice_seconds:voiceSeconds,p_default_messages:BASIC_USAGE_LIMITS.messagesPerMonth,p_default_voice_seconds:BASIC_USAGE_LIMITS.voiceSecondsPerMonth,p_default_warning_percent:BASIC_USAGE_LIMITS.warningPercent});
