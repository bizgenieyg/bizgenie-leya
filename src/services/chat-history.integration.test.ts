...
async function fixture(history: WhatsAppChatMessage[] | Error) {
  ...
  const send = async (id: string, text: string) => {
    ...
    await handleWebhookEvent(tenantId, { event: 'message', payload: { id, from: lid, fromMe: false, hasMedia: false, body: text, author: null, replyTo: null, _data: { Info: { PushName: 'Тест' } } } }, db, provider, ai as never);
  };
...
test('first contact loads WhatsApp history once, without re-introduction or persistence', async () => {
  ...
  try {
    await f.send(`false_${lid}_NOW`, 'Когда вы открыты?');
    ...
    await f.send(`false_${lid}_NEXT`, 'А в пятницу?');
