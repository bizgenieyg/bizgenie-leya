...
adminRouter.post('/simulator/owner-answer',async(request,response)=>{const tenantId=queryTenantId(request.query.tenantId),body=objectBody(request.body),answer=requiredString(body,'answer').trim(),sessionId=requiredString(body,'sessionId').trim();if(!answer||answer.length>2000)throw new HttpError(400,'Answer must contain 1 to 2000 characters',{code:'simulator_invalid_message'});if(!isUuid(sessionId))throw new HttpError(400,'sessionId must be a UUID',{code:'simulator_invalid_session'});response.setHeader('Cache-Control','no-store');try{response.json(await simulateOwnerAnswer(supabase,tenantId,sessionId,answer));}catch(error){console.error('simulator_owner_answer_failed',{tenantId,errorName:error instanceof Error?error.name:'unknown'});if(error instanceof HttpError)throw error;throw new HttpError(503,'Simulator unavailable',{code:'simulator_processing_unavailable'});}});
