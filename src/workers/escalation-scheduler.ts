...
export function startEscalationScheduler() {
  let busy=false;
  const tick=async()=>{if(busy)return;busy=true;try{await runDueScheduledEscalations(supabase,createWhatsAppProvider);}catch{console.error('escalation_scheduler_failed');}finally{busy=false;}};
